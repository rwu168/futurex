    ...
    /*=================Display detail of trading activities==================================*/
        function disp_detail($conn,$today)
        {
                ...
                while ($k<=$rowcount)
                {
                    ...
                    $pl=$pl-($qty1*6);
                    //print($name);
                    $sql1 = "Select qty,amt,bal,mkt_cond,mnq,rge,micro,contracts,rty,spriceselldown,sprice,mmy,secbuy,s1,f1,seccont,seccont1,ym,ym1,flag2,reserve,s2,rpl,seccontqty,seccont1qty,ramt,spriceselldown,m2k from control where sys='$name';";
                    ...
                    $micro=$row1[7];
                    //echo $name .$bal;
                    $tsymbol=$row1[13];$urper=round($row1[14],2);$seccont=$row1[15];$seccont1=$row1[16];$ym=$row1[17];$ym1=$row1[18];$tradeclass=$row1[19];$reserve=$row1[20];$ramt=$row1[21];$pivotqty=$row1[22];$seccontqty=$row1[23];$seccont1qty=$row1[24];$ramt1=$row1[25];$hedgecap=$row1[26];$m2k=$row1[27];
                    ...
                    if ($rowcount1>0)
                    {
                        ...
                        */
                        ...
                        $data=array_merge($data,array($name,$pl,$rpl,$bal,$per,$qty,$level,$mkt_cond,$rge,$micro,$contracts,$ramt1,$urper,$sprice,$seccont,$seccontqty,$ym,$seccont1,$seccont1qty,$ym1,$ramt,$pivotqty,$m2k,$rge1,$hedgecap));
                    }
                    $row=pg_fetch_row($rs);
                    $k++;
